...
export function StatusPreviewCard({ text, imageUri, audioUri, location, emojis, backgroundColor }: Props) {
  ...
  return (
    <View style={[styles.card, { borderColor: palette.border, backgroundColor: palette.surface }]}> 
      <Text style={[styles.title, { color: palette.textPrimary }]}>Vista previa</Text>
      <View style={[styles.previewBody, { backgroundColor: hasMedia ? (isDark ? '#121D2B' : '#F4F8FF') : backgroundColor }]}> 
        ...
        {text.trim() ? <Text style={[styles.text, { color: hasMedia ? palette.textPrimary : '#FFFFFF' }]}>{text.trim()}</Text> : null}
        {!!emojis.length ? <Text style={styles.emojiText}>{emojis.join(' ')}</Text> : null}
        {location ? (
          <View style={styles.metaRow}><Ionicons name="location" size={14} color={palette.accent} /><Text style={[styles.metaText, { color: palette.textSecondary }]}>Ubicación adjunta</Text></View>
        ) : null}
        {audioUri ? (
          <View style={styles.metaRow}><Ionicons name="mic" size={14} color={palette.accent} /><Text style={[styles.metaText, { color: palette.textSecondary }]}>Audio listo para publicar</Text></View>
        ) : null}
        {!text.trim() && !imageUri && !audioUri && !location && !emojis.length ? (
          <Text style={[styles.empty, { color: '#EAF3FF' }]}>Tu estado aparecerá aquí.</Text>
        ) : null}
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  text: { fontSize: 18, fontWeight: '600', lineHeight: 24 },
  emojiText: { fontSize: 24 },
  metaRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  metaText: { fontSize: 13, fontWeight: '600' },
  empty: { textAlign: 'center', fontSize: 14, fontWeight: '600' },
});
